...
import { createInsert, createUpdate, TableColumn, whereParams } from "@database/helpers/queryGenerators";
...
class ClienteRepository extends GenericTableManager {
  name?: string;
  email?: string;
  phone?: string;
  date?: string;
  ...
  constructor({ id, name, email, phone, date, comments, statusId }: ClienteProps) {
    super("cliente", { id });
    this.name = name;
    this.email = email;
    this.phone = phone;
  ...
  static async saveAll(clientes = []) {
    const columns = ["id", "name", "email", "phone", "date", "comments", "status_id"];
    return await GenericTableManager.saveAll("cliente", clientes, columns);
  }
  ...
  static async get({ id, name, email, phone, date, comments, statusId }: ClienteProps, conditional = "and") {
    const { params, whereParams } = ClienteRepository.createWhereParams({
  ...
  static async getByText({ text, offset = 0, limit = 10 }) {
    const sql = `
      ...
      LIMIT ? OFFSET ?;
    `;
    const queryParams = [`%${text}%`, `%${text}%`, `%${text}%`, `%${text}%`, `%${text}%`, limit, offset];
  ...
  static async updateById(id: string, { name, email, phone, date, comments, statusId }: ClienteProps) {
    const { query, queryParams } = ClienteRepository.createUpdate({ name, email, phone, date, comments, statusId }, "id = ?");
    queryParams.push(id);
  ...
  static createWhereParams({ id, name, email, phone, date, comments, statusId }: ClienteProps, conditional = "and") {
    const preParams = [
  ...
  static createInsert({ id, name, email, phone, date, comments, statusId }: ClienteProps) {
    const columns = [
  ...
  static createUpdate({ id, name, email, phone, date, comments, statusId }: CreateClienteProps, condition = "id = ?") {
    const columns = [
      new TableColumn("id", id),
      new TableColumn("name", name),
      new TableColumn("email", email),
