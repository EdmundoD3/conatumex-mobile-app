...
export default function Login() {
  ...
  const handleQuickStart = () => {
    // Lógica para saltar el login en desarrollo
    const date = new Date()
    const dateTeenMinutes = new Date(date.getTime() + 30*6000) 
    const dateHour = new Date(date.getTime() + 60*6000) 
    const userSession = new UserSession({
      username:"username",
      user:"user",
      token:new Token({expiryDate:dateTeenMinutes,token:"askfhaisfaos"}),
      refreshToken: new Token({expiryDate:dateHour,token:"askfhaisfaos"}),
      key:"false"
    })
    login({userSession,key:"false"})
  };
    ...
    >
      <View style={styles.loginContainer}>
        ...
        <TextInput
          ...
          onChangeText={(text) =>
            setUserData((data) => ({ ...data, username: text }))
          }
        ...
        {load ? (
          ...
          <TouchableOpacity
            style={styles.button}
            onPress={() => handleLogin(loginArguments)}
          >
